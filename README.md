...
| `registerUrl` | “报名入口”按钮的外链；**留空 `""` 会自动回退到百度** |
| `detail` | **详情页数据**（点击卡片 / 查看详情后展示）；见下方“详情页” |

> 链接回退逻辑：`registerUrl` 为空时，前端就用 `FALLBACK_URL`（百度）。
> 拿到真实报名链接后，把 `registerUrl` 填上即可，无需改前端代码。

### 页面导航（单页应用，无需刷新）

- 点击**卡片**或**「查看详情」**按钮 → 切换到该竞赛的**详情页**。
- 详情页左上角**「← 返回推荐列表」** → 切回刚才的推荐列表，列表状态原样保留。
- 全程在同一个 `index.html` 内切换视图（`showDetail(i)` / `showList()`），不跳转、不重新加载。
- 只有「报名入口」是真正的外链（跳 `registerUrl`，暂为百度）。

### 详情页数据（`competitions[].detail`）
  ...
  "sections": [
    {
      ...
      "title": "基本信息",
      "rows": [
        { "label": "竞赛全称", "value": "……（C4-AI）" },
        { "label": "竞赛级别", "value": "一级（推免加分 6 分）", "highlight": true },
        { "label": "报名截止", "value": "2026-06-30 23:59" }
      ]
    }
  ]
}
...
- `tags`：详情页顶部紫色区的标签胶囊，`icon` + `label`。
- `sections[]`：可放多个信息分区（如“基本信息”“赛程安排”），每个分区有 `title` 和若干 `rows`。
- `rows[].highlight: true`：该值用蓝色高亮（如竞赛级别）。
- `sections` 是通用结构，想加更多字段/分区直接往里加，详情页会自动渲染。
